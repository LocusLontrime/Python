...
def parse_molecule(formula: str):
    ...
    def rec_parser(ind: int, depth: int, tab_: str):
        tab = tab_ + "|   "
        ...
        while i < l_:
            ...
            if i < l_ and (symb := formula[i]) in '{[(':
                ...
                i, inner_atoms = rec_parser(i + 1, depth + 1, tab)
                temp_ind = i
                while i < l_ and formula[i].isdigit():
                    i += 1
                ...
                print(f'{tab[:-4]}<<-- closing bracket ...{symb} ind: {ind=} {i=}')
                return i + 1, atoms
        return atoms
    return dict(rec_parser(0, 0, ''))


print(f'PARSING molecules to atoms: ')
