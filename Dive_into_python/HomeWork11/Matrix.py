class Matrix:
    ...
    def __init__(self, matrix: list[list[int]]):
        ...
        for row in matrix:
            set_.add(len(row))
            if len(set_) > 1:
                raise ValueError(f"Matrix' rows must have the same lengths!")
        self.matrix = matrix
    ...
    def core(self, other: 'Matrix', sign_plus: bool = True):
        if not self.check_sizes(other):
            raise ValueError(f'Matrices must have the same sizes!')
    ...
    def __mul__(self, other: 'Matrix'):
        if (k := len(self.matrix[0])) != len(other.matrix):
            raise ValueError(f'Rows quantity of the left matrix should be equal to Columns quantity of the right one')
