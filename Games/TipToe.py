...
class Vertex:
    ...
    def get_rot_dir(self, twin: 'Vertex'):
        ...
        self._rot_dir = RotDir.CLOCKWISE if Pathfinder.cross_product_val(*l_vector,
                                                                         *r_vector) < 0 else RotDir.ANTI_CLOCKWISE
...
class Pathfinder(arcade.Window):
    ...
    # par, locking other funcs while circle is being drawn:
    _in_interaction = False

    # par for showing vertexes (v_hashes)
    _show_v_hashes = False

    def __init__(self, width: int, height: int, a: Point, b: Point):
        ...
        # v_hashes visualization:
        self.v_hashes_shapes = None
        self.setup()
        self.g = None
        # interaction pars:
        self.center = None
    ...
    def setup(self):
        ...
        self.circles = []
        self.circle_shapes = arcade.ShapeElementList()

        # start point:
        a = arcade.create_ellipse(q * self.start.x + dx, q * self.start.y + dy, 2 * 10, 2 * 10, arcade.color.GREEN_YELLOW)

        # end point:
        b = arcade.create_ellipse(q * self.end.x + dx, q * self.end.y + dy, 2 * 10, 2 * 10, arcade.color.JUNGLE_GREEN)

        self.circle_shapes.append(a)
        self.circle_shapes.append(b)

        # v_hashes:
        self.v_hashes_shapes: arcade.ShapeElementList = arcade.ShapeElementList()
    ...
    def on_draw(self):
        ...
        if self._in_interaction:
            if self.center and self.rad:
                arcade.draw_circle_filled(self.center.x, self.center.y, self.rad, arcade.color.BROWN)  # while we are adding some more circles...
        ...
        # v_hashes:
        if self._show_v_hashes:
            if self.v_hashes_shapes:
                self.v_hashes_shapes.draw()
    ...
    def on_key_press(self, symbol: int, modifiers: int):
        ...
        match symbol:
            ...
            case arcade.key.SPACE:
                self.g = self.shortest_path_length(self.start, self.end, flag=False)
            case arcade.key.Z:
                ...
            case arcade.key.Y:
                ...
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int):
        if button == 1 and not self._in_interaction:
            print(f'mouse button [{button}] been pressed...')
            self.center = Point(x, y)
            self.rad = 0
            self._in_interaction = True
        elif button == 2:
            self._show_v_hashes = not self._show_v_hashes

    def on_mouse_motion(self, x: int, y: int, dx: int, dy: int):
        if self._in_interaction:
            self.rad = math.hypot(abs(self.center.x - x), abs(self.center.y - y))

    def on_mouse_release(self, x: int, y: int, button: int, modifiers: int):
        if button == 1 and self._in_interaction:
            # print(f'{self.center = }')
            # print(f'{self.rad = }')
            self.add_circle(self.center, self.rad)
    ...
    def shortest_path_length(self, a: Point, b: Point, flag=False) -> float:
        ...
        if not self.validate_ab(a, b, self.circles):
        ...
        print(f'There are {len(circles)} different valuable circles...')
        ...
        # v_hashes shape creating:
        vertex: Vertex
        for vertex in v_hashes.values():
            x = vertex.x * Pathfinder.MULTIPLIER + Pathfinder.DELTA_X
            y = vertex.y * Pathfinder.MULTIPLIER + Pathfinder.DELTA_Y
            self.v_hashes_shapes.append(arcade.create_ellipse(x, y, 4, 4, arcade.color.RED))
    ...
    def get_valid_edges(self, c1_: Circle, c2_: Circle, circles: list[Circle], flag: bool = False) -> (
    ...
    #interactive section:
    def add_circle(self, center: Point, rad: float):
        center_ = Point(
            (center.x - Pathfinder.DELTA_X) / Pathfinder.MULTIPLIER,
            (center.y - Pathfinder.DELTA_Y) / Pathfinder.MULTIPLIER
        )
        rad_ = rad / Pathfinder.MULTIPLIER
        self.circles += [circle_ := Circle(ctr=center_, r=rad_)]
        # print(f'{self.circles = }')
        # print(f'{circle_ = }')
        shape_ = arcade.create_ellipse(center.x, center.y, 2 * rad, 2 * rad, arcade.color.BLUE_GRAY)
        self.circle_shapes.append(shape_)
...
def main():
    ...
    game = Pathfinder(SCREEN_WIDTH, SCREEN_HEIGHT, Point(1, 1), Point(13, 6))
    arcade.run()
