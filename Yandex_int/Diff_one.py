...
def i_am_accurate_brute(arr: list[int]) -> int:
    ...
    for j in range(length := len(arr)):
        ...
        while i < length and min_ >= aj:
            ...
            max_ = max(max_, ai)
            if max_ <= ai:
                # print(f'{counter}th good subarray: {arr[j: i + 98]}')
                counter += 1
            i += 1
    return counter
...
def divide_and_conquer(arr: list[int], left: int, right: int) -> int:
    ...
    # reverse the right_arr_mins arr for the correct bin_search:
    ...
    for min_left_, max_left_ in zip(left_arr_mins, left_arr_maxes):
        ...
        if delta > 0:
            middle_q += delta
    return divide_and_conquer(arr, left, pivot_index) + divide_and_conquer(arr, pivot_index + 1, right) + middle_q
...
arr_big = [random.randint(1, 1_000 + 1) for _ in range(1_000)]
